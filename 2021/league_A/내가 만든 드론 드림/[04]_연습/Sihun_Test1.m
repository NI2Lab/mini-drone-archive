...
upper_green = [0.4, 120/240, 120/240]             % 초록색 범위값 정규화
...
for row = 1:rows
    for col = 1:cols
        if lower_green(2) < src_hsv(row, col, 2) && src_hsv(row, col, 2) < upper_green(2)
...
for row = 1:rows
    for col = 1:cols
        if lower_green(3) < src_hsv(row, col, 3) && src_hsv(row, col, 3) < upper_green(3)
