...
# 대회 진행 전략  
  ...
  드론으로 찍은 사진을 통해 객체를 분석하여 가장 큰 객체의 중점을 인식하고 중점으로 이동하는 원리를 사용하였다.  
...
# 알고리즘 설명  
  ...
  파란색 천을 흰색으로 만든다음 흰색부분의 중심을 찾아 드론이 그 중심의 위치를 움직이게 한다.
  그리고 흰색중심과 드론의 중심이 거의 같아질 경우 전진을 하여 천을 통과한다 그리고 색을 감지해 다음 행동을 하게 한다.
  색을 감지하면 색에 맞는 드론의 움직임을 제어한다. 색을 판단하는 방법은 hsv이미지의 값을 읽으면 된다. 예를들어 빨간색일때는 hsv(i,j,1), hsv(i,j,2), hsv(i,j,3)의 값이 [1   0 0]이다. 이렇게 색을 감지하여 if문을 통해 드론의 움직임을 제어한다.    
...
for i=1:size(hsv,1)
    ...
    if hsv(i,j,1)==1&&hsv(i,j,2)==0&&hsv(i,j,3)==0 % 빨간색이면 if문 실행 RED일때 = [1, 0, 0], GREEN일때 = [0,1,0]
            % 각 단계마다 if문 조건식 수정하면 됨
    else
        hsv(i,j,1)=0; % 빨간색이 아닌 경우 hsv의 값을 0으로 변경
        hsv(i,j,2)=0;
        hsv(i,j,3)=0;
    end
...
w_z=center(1,2);
    ...
    if hsv(int16(w_z),int16(w_y),1)==1&&hsv(int16(w_z),int16(w_y),2)==0&&hsv(int16(w_z),int16(w_y),3)==0  % 빨간색일때
...
for i=1:size(hsv,1)
    for j = 1:size(hsv,2)
    if hsv(i,j,1)==0&&hsv(i,j,2)==1&&hsv(i,j,3)==0 % 초록색일때 if문 실행 
    else
        hsv(i,j,1)=0;
        hsv(i,j,2)=0;
        hsv(i,j,3)=0;
...
d_center=[size(BW,2)./2, size(BW,1)./2];

subimage(hsv)

    if hsv(int16(w_z),int16(w_y),1)==0&&hsv(int16(w_z),int16(w_y),2)==1&&hsv(int16(w_z),int16(w_y),3)==0  % 초록색일때
    turn(droneObj,deg2rad(90));% 시계방향으로 90도 회전
    move(droneObj,[1 0 0],'Speed',0.5);
...
for i=1:size(hsv,1)
    for j = 1:size(hsv,2)
    if hsv(i,j,1)==1&&hsv(i,j,2)==0&&hsv(i,j,3)==1 % 빨간색이면 if문 실행 RED일때 = [1, 0, 0], GREEN일때 = [0,1,0]
            % 각 단계마다 if문 조건식 수정하면 됨
    else
        hsv(i,j,1)=0;
        hsv(i,j,2)=0;
...
subimage(hsv)
    ...
    if hsv(int16(w_z),int16(w_y),1)==1&&hsv(int16(w_z),int16(w_y),2)==0&&hsv(int16(w_z),int16(w_y),3)==1  % 보라색일때
    land(droneObj);  %드론 착륙
    break; % while탈출
    end
end %while문의 end
